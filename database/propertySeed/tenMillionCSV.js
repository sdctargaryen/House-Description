...
generateProperty = (i) => {
  return {
    ...
    amenities: `"{
      ...
      iconUrl: ${xRandomElements(amenIcons, randomElement([2, 3, 4]))}
    }"`,
    numBaths: Math.ceil(Math.random() * 4),
    host: `"{
      name: ${randomElement(names)}, 
      pictureUrl: ${randomElement(hostImgs)}
    }"`,
...
var endTime;
let header = "id, propertyInfo, beds, amenities, numBaths, host, summary, __v \n";
...
function writeTenMillionTimes() {
  ...
  function write() {
    ...
    while (i > 0 && ok) {
      if(i === 10) {
        file.write(header, "UTF8");
      }
      i--;
      endTime = new Date();
      let timeDiff = (endTime - startTime) / 1000;
      if (i%100000 === 0) console.log(`${(1e7 - i)/1000000}M of obj created at ${timeDiff} sec`);

      var contentStr = JSON.stringify(Object.values(generateProperty(i)));
      contentStr = contentStr.substring(1, contentStr.length-1) + '\n';
      
      if (i === 0) {
        // last time!
        file.write(contentStr, "UTF8", ()=>console.log("finish seeding"));
      } else {
        // See if we should continue, or wait.
        // Don't pass the callback, because we're not done yet.
        
        ok = file.write(contentStr, "UTF8");
      }
    }
